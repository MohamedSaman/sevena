<div>
    ...
    <div class="p-2">
                    ...
                    <tbody>
                        @forelse ($loans as $loan)
                            <tr>
                                ...
                                <td class="px-4 py-3">
                                    <button wire:click="showLoanDetails({{ $loan->loan_id }})"
                                        class="px-3 py-1 my-1 bg-blue-600 text-white rounded hover:bg-blue-700 text-sm">
                                        View Details
                                    </button>
                                    @if ($loan->status === 'active')
                                        <button wire:click="markAsPaid({{ $loan->loan_id }})" class="btn-outline px-4 py-1 my-1 rounded-lg border border-blue-600 text-blue-600 hover:bg-blue-50">Mark as Paid</button>
                                    @endif
                                </td>
                            </tr>
                        @empty
        ...
        <!-- Loan Details Modal -->
        @if($showLoanDetailsModal)
            <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
                <div class="bg-white rounded-lg w-full max-w-4xl mx-4 overflow-auto max-h-[90vh]">
                    <div class="p-6">
                        <h3 class="text-lg text-center font-semibold mb-4">Loan Details</h3>
                        
                        <div id="loan-details-content">
                            <!-- Loan Header -->
                            <h1 class="fs-4 fw-bold text-center my-4">Loan Statement</h1>
                            <div class="payslip-header d-flex justify-content-between align-items-center mb-6">
                                <div>
                                ...
                                <div class="payslip-logo">
                                    <img src="https://sevena.lk/img/logo.png" alt="Logo" class="img-fluid" style="max-height: 50px;">
                                </div>
                            </div>

                            <!-- Employee Info -->
                            <div class="employee-info mb-4 flex justify-between">
                                <div>
                                    <p><strong>Employee:</strong> {{ $loanDetails['employee_name'] ?? 'Unknown' }}</p>
                                    <p>EMP ID: {{ $loanDetails['employee_id'] ?? '' }}</p>
                                    <p>Designation: {{ $loanDetails['designation'] ?? 'Employee' }}</p>
                                </div>
                                <div class="text-right">
                                    <p><strong>Loan Start:</strong> {{ $loanDetails['start_date'] ?? '' }}</p>
                                    <p>Loan ID: {{ $loanDetails['loan_id'] ?? '' }}</p>
                                </div>
                            </div>

                            <!-- Loan Details Table -->
                            <table class="payslip-table w-full mb-4">
                                <thead>
                                    <tr>
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <td class="text-right">LKR {{ number_format($loanDetails['loan_amount'] ?? 0, 2) }}</td>
                                    </tr>
                                    <tr>
                                        <td></td>
                                        <td>Interest Rate</td>
                                        <td class="text-right">{{ number_format($loanDetails['interest_rate'] ?? 0, 2) }}%</td>
                                    </tr>
                                    <tr>
                                        <td></td>
                                        <td>Term (Months)</td>
                                        <td class="text-right">{{ $loanDetails['term_month'] ?? 0 }}</td>
                                    </tr>
                                    <tr>
                                        <td></td>
                                        <td>Start Date</td>
                                        <td class="text-right">{{ $loanDetails['start_date_full'] ?? '' }}</td>
                                    </tr>
                                    <tr class="bg-gray-50 font-semibold">
                                        <td></td>
                                        <td>Monthly Payment</td>
                                        <td class="text-right">LKR {{ number_format($loanDetails['monthly_payment'] ?? 0, 2) }}</td>
                                    </tr>
                                    <tr class="bg-gray-50 font-semibold">
                                        <td></td>
                                        <td>Remaining Balance</td>
                                        <td class="text-right">LKR {{ number_format($loanDetails['remaining_balance'] ?? 0, 2) }}</td>
                                    </tr>
                                </tbody>
                            </table>

                            <!-- Monthly Payment History -->
                            <h3 class="font-semibold mb-1 mt-4">Payment History</h3>
                            <table class="payslip-table w-full">
                                <thead>
                                    <tr>
                                        <th class="text-left">Payment Date</th>
                                        <th class="text-right">Amount Paid (LKR)</th>
                                        <th class="text-right">Remaining Balance (LKR)</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @forelse($loanDetails['payment_history'] ?? [] as $payment)
                                        <tr>
                                            ...
                                            <td class="text-right">{{ number_format($payment['remaining_balance'], 2) }}</td>
                                        </tr>
                                    @empty
                                        <tr>
                                            <td colspan="3" class="text-center py-2">No payment history available</td>
                                        </tr>
                                    @endforelse
                                    @if(!empty($loanDetails['payment_history']))
                                        <tr class="bg-green-100 font-semibold">
                                            <td>Total Paid</td>
                                            <td class="text-right">LKR {{ number_format($loanDetails['total_paid'] ?? 0, 2) }}</td>
                        ...
                        <div class="flex justify-end mt-8">
                            <button wire:click="closeLoanDetails" class="px-6 py-2 bg-gray-500 text-white rounded mr-3">Close</button>
                            <button onclick="printLoanDetails()" class="px-4 py-2 bg-green-600 text-white rounded">Print Loan Details</button>
                        </div>
                    </div>
                </div>
            </div>
